...
switch() {
    ...
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        ...
        light_bg_color=$(jq -r '.appearance.palette.lightBackgroundColor' "$SHELL_CONFIG_FILE")
        ...
        [[ "$light_bg_color" != "null" && -n "$light_bg_color" ]] && generate_colors_material_args+=(--light_bg_color "$light_bg_color")
    fi
    ...
    if [[ "$mode_flag" == "light" && -n "$light_bg_color" && "$light_bg_color" != "null" ]]; then
        # Validate the color format before applying
        if [[ ! "$light_bg_color" =~ ^#[0-9A-Fa-f]{6}$ ]]; then
            echo "[switchwall.sh] Warning: Invalid color format '$light_bg_color', skipping override" >&2
        else
            ...
            if [ -f "$gtk3_css" ]; then
                ...
                # Escape special characters for sed and replace window_bg_color with custom color
                escaped_color=$(printf '%s\n' "$light_bg_color" | sed 's/[[\.*^$/]/\\&/g')
                sed -i "s/@define-color window_bg_color #[0-9A-Fa-f]\{6\};/@define-color window_bg_color ${escaped_color};/g" "$gtk3_css"
            fi
            ...
            if [ -f "$gtk4_css" ]; then
                ...
                # Escape special characters for sed and replace window_bg_color with custom color
                escaped_color=$(printf '%s\n' "$light_bg_color" | sed 's/[[\.*^$/]/\\&/g')
                sed -i "s/@define-color window_bg_color #[0-9A-Fa-f]\{6\};/@define-color window_bg_color ${escaped_color};/g" "$gtk4_css"
            fi
        fi
    fi
